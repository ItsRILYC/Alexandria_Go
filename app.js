...
// Toon het juiste “scherm” + render data waar nodig
const showScreen = (screenId) => {
  ...
  // Render specifieke data bij elk scherm
  if (screenId === 'training-screen') {
    renderTraining();
    updateLiveStats();
  } else if (screenId === 'roster-screen') {
    renderRoster();
  } else if (screenId === 'seasons-screen') {
    renderSeasons();
  }
};
...
// Render detail van een specifiek seizoen met statistieken (alleen percentages, geen totale aantallen)
const renderSeasonDetail = (seasonIndex) => {
  ...
  // Totale percentages (zonder absolute aantallen)
